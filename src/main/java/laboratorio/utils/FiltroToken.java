...
public class FiltroToken extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain
        ...
        if (req.getMethod().equals("OPTIONS")) {
            ...
            boolean error = true;
        try{
            if (requestURI.startsWith("/api/digitador") || requestURI.startsWith("/api/ingeniero")

                    || requestURI.startsWith("/api/administrador") ) {
                if(token != null) {
                    Jws<Claims> jws = jwtUtils.parseJwt(token);
                    if (
                            ( requestURI.startsWith("/api/digitador") &&

                                    !jws.getBody().get("rol").equals("digitador") ) ||

                                    ( requestURI.startsWith("/api/ingeniero") &&

                                            !jws.getBody().get("rol").equals("ingeniero") ) ||

                                    ( requestURI.startsWith("/api/administrador") &&

                                            !jws.getBody().get("rol").equals("administrador") )) {
